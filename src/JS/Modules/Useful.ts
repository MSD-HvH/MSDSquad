...
exports.Cheat = {
    ...
    PrintColor: function (text, color, dev) {
        ...
        if (!color || typeof color != "object" || color.length < 4 || color.length > 4)
            return throw_error(
                "Cheat.PrintColor()",
                "To use this method, your color must be an Array with 4 elements: Red, Green, Blue, Alpha"
            );

        if (typeof text == "object")
            return Cheat.PrintColor(color, JSON.stringify(text, null, 2) + (dev ? ": " + typeof text + "\n" : "\n"));
...
exports.Math = {
    ...
    NormalizeAngles: function (angles) {
        ...
        if (angles.z > 0) angles.zangles.z = 0.0;
    },
};
        ...
        if (h < w) {
            for (i = 0; i < h; i++) {
                Render.GradientRect(
                    ...
                    1,
                    1,
                    this.LerpColor(i / h, top_left, bottom_left),
                    this.LerpColor(i / h, top_right, bottom_right)
                );
            }
        } else {
            for (i = 0; i < w; i++) {
                Render.GradientRect(
                    ...
                    h,
                    0,
                    this.LerpColor(i / w, top_left, top_right),
                    this.LerpColor(i / w, bottom_left, bottom_right)
                );
            }
        }
    },
