...
function get_player() {
    ...
    if(attacker == local && victim !== local) {
        ...
        logs.push(["[", "onetap", "] hit ", name + "'s ", hitgroup, " for ", damage, " (", health, ") ", hitchance, " hc", " (", safepoint, ":", exploit + 1, ")"])
    ...
    if(UI.GetValue(["Visuals", "Logs", "Logs", "Type"]) & (1 << 0)) {
        for(var i in logs) {
            Cheat.PrintColor([255, 255, 255, 255], logs[i][0]); Cheat.PrintColor([250, 166, 24, 255], logs[i][1]); Cheat.PrintColor([255, 255, 255, 255], logs[i][2]); Cheat.PrintColor([255, 255, 255, 255], logs[i][3]); Cheat.PrintColor([250, 166, 24, 255], logs[i][4]); Cheat.PrintColor([255, 255, 255, 255], logs[i][5]); Cheat.PrintColor([250, 166, 24, 255], logs[i][6].toString()); Cheat.PrintColor([255, 255, 255, 255], logs[i][7]); Cheat.PrintColor([250, 166, 24, 255], logs[i][8].toString()); Cheat.PrintColor([255, 255, 255, 255], logs[i][9]); Cheat.PrintColor([250, 166, 24, 255], logs[i][10].toString()); Cheat.PrintColor([255, 255, 255, 255], logs[i][11]); Cheat.PrintColor([255, 255, 255, 255], logs[i][12]); Cheat.PrintColor([250, 166, 24, 255], logs[i][13].toString()); Cheat.PrintColor([255, 255, 255, 255], logs[i][14]); Cheat.PrintColor([250, 166, 24, 255], logs[i][15].toString()); Cheat.PrintColor([255, 255, 255, 255], logs[i][16] + "\n")
        }
    }
}
